    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex space-x-1 bg-gray-100 rounded-lg p-1">
            <button
                ...
                class="px-4 py-2 text-sm font-medium rounded-md transition-colors {{ $filter === 'all' ? 'bg-white text-gray-900 shadow' : 'text-gray-600 hover:text-gray-900' }}"
            >
                All
            </button>
            <button
                wire:click="$set('filter', 'unread')"
                class="px-4 py-2 text-sm font-medium rounded-md transition-colors {{ $filter === 'unread' ? 'bg-white text-gray-900 shadow' : 'text-gray-600 hover:text-gray-900' }}"
            >
                Unread
            </button>
            <button
                wire:click="$set('filter', 'read')"
                class="px-4 py-2 text-sm font-medium rounded-md transition-colors {{ $filter === 'read' ? 'bg-white text-gray-900 shadow' : 'text-gray-600 hover:text-gray-900' }}"
            >
                Read
            </button>
        </div>
        ...
        <div>
            ...
            >
                ...
                @foreach($notificationTypes as $type)
                    <option value="{{ is_array($type) ? $type['value'] ?? $type : $type }}">{{ ucfirst(is_array($type) ? $type['value'] ?? $type : $type) }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    @else
        ...
        <div class="space-y-6">
            @foreach($grouped as $type => $group)
                <div>
                    <h2 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 px-1">{{ ucfirst($type) }}</h2>
                    <div class="bg-white shadow rounded-lg divide-y divide-gray-200">
                        @foreach($group as $notification)
                            <div class="flex items-start px-4 py-4 {{ empty($notification['read_at']) ? 'bg-indigo-50' : '' }}">
                                {{-- Icon --}}
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm {{ empty($notification['read_at']) ? 'font-bold text-gray-900' : 'font-medium text-gray-700' }}">
                                        {{ $notification['title'] ?? '' }}
                                    </p>
